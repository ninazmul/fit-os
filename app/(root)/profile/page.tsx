    ...
    <div className="space-y-6">
      ...
      <div className="glass-card p-6 rounded-3xl border border-border/50 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-2xl font-bold tracking-tight">{profile?.name || "User Profile"}</h1>
            <p className="text-xs text-muted-foreground capitalize">
              {profile?.gender} &middot; {profile?.age} yrs &middot; {profile?.height} cm
            </p>
          </div>
        </div>
      ...
      <div className="glass-card p-6 rounded-3xl border border-border/50 space-y-4">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 text-xs">
          <div className="p-3 rounded-2xl bg-muted/40 border border-border/30">
            ...
            <p className="text-lg font-bold text-emerald-600 dark:text-emerald-400 mt-1">{profile?.dailyProteinGoal} g</p>
          </div>

          <div className="p-3 rounded-2xl bg-muted/40 border border-border/30">
            <p className="text-muted-foreground font-medium">Carbs Target</p>
            <p className="text-lg font-bold text-blue-600 dark:text-blue-400 mt-1">{profile?.dailyCarbGoal} g</p>
          </div>

          <div className="p-3 rounded-2xl bg-muted/40 border border-border/30">
            <p className="text-muted-foreground font-medium">Fat Target</p>
            <p className="text-lg font-bold text-purple-600 dark:text-purple-400 mt-1">{profile?.dailyFatGoal} g</p>
          </div>

          <div className="p-3 rounded-2xl bg-muted/40 border border-border/30">
            <p className="text-muted-foreground font-medium">Fiber Target</p>
            <p className="text-lg font-bold text-amber-600 dark:text-amber-400 mt-1">{profile?.dailyFiberGoal} g</p>
          </div>
        </div>
      </div>
    </div>
